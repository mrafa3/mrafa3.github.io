...
In this post, I'd like to drill down further -- to the Census tract level -- to demonstrate how to fetch, wrangle, and visualize data using `tidycensus::`.
...
As I did in the prior demos, I'll use data from the American Community Survey (ACS). This time, I'll show how to fetch data at the **table** level (as opposed to prior posts where I've pulled individual variables).

I'd like to analyze and map data related to children and health insurance coverage. Using the `tidycensus::load_variables()` function and searching on "health insurance" shows all variables that include that in the American Community Survey.
...
This search led me to table "B27001" (you can identify tables using the `name` column and all characters prior to the underscore). Now, in the `get_acs()` function, I'll use the `table=` argument instead of the `variables=`.

```{r health_insurance_vars, include=TRUE, echo=FALSE}
...
```{r acs_data_block_group2, include=TRUE}
