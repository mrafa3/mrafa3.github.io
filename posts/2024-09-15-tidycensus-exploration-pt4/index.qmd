...
```{r agg_combined_acs_data, include=TRUE, echo=FALSE}
(agg_combined_acs_data <- combined_acs_data %>% 
  as.data.frame() %>% 
  select(-geometry) %>% 
  group_by(year) %>% 
  summarise(ttl_worked_from_homeE = sum(ttl_worked_from_homeE),
            ttl_worked_from_homeM = moe_sum(ttl_worked_from_homeM, estimate = ttl_worked_from_homeE)) %>% 
  mutate(annual_perc_wfh_change = (ttl_worked_from_homeE - lag(ttl_worked_from_homeE, 1)) / 
                                     lag(ttl_worked_from_homeE, 1)))
```


```{r}
(fig1 <- agg_combined_acs_data %>% 
  ggplot(.,
         aes(x=year,
...
agg_combined_acs_data %>% 
  select(-ttl_worked_from_homeM) %>% 
  gt() %>% 
  fmt_number(columns = ttl_worked_from_homeE, decimals = 0) %>% 
  fmt_percent(columns = annual_perc_wfh_change, decimals = 1) %>% 
  gt::data_color(
    columns = annual_perc_wfh_change
  )
```
