...
ttl_passing_stats_99_24 %>% filter(season == 2024) %>% arrange(-passing_yards_per_game)
```
...
What we're seeing is a "higher floor, lower cieling" league as far as passing goes. **Only one team in 2024 averaged less than 200 yards passing (Patriots), while no teams average above 300.**
...
Joe Burrow and the Bengals were *cooking* on offense in 2024. They are averaged {r top_2024_passing_team} yards per game, which would rank in the {r top_2024_passing_team_percentile}th percentile in the last 25 years.
...
```{r season_passing_league_column, fig.width=9}
passing_yards_season_average %>% 
  ggplot(.,
...
ball_carrier_epa %>% 
  ...
  # geom_text()
  ggrepel::geom_text_repel(data=. %>% filter(touch == ttl_touch) %>% slice_max(cumsum_epa, n = 8),
                           aes(label=ball_carrier),
                           box.padding = .5,
                           nudge_x = 1,
                           force = 10) +
  labs(title='Top WR value is much higher than top RBs',
       subtitle='Minimum 125 touches',
       caption='**Source**: {nflverse}',
       x='Touch #',
