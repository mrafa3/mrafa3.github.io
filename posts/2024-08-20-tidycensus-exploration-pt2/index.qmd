...
```{r plot_margin_error, include=FALSE}
combined_acs_data %>% 
  filter(year == 2022) %>% 
  # filter(year %in% c(2018, 2019, 2020, 2021, 2022)) %>% 
  # ggplot(.,
  #        aes(x=u5_male_pov_estimate,
  #            y=reorder(county, u5_male_pov_estimate))) + 
  # geom_point() + 
  # geom_errorbarh(aes(xmin = u5_male_pov_estimate - u5_male_pov_moe, 
  #                    xmax = u5_male_pov_estimate + u5_male_pov_moe), height = 0.2) + 
  ggplot(.,
         aes(x=u5_female_pov_estimate,
             y=reorder(county, u5_female_pov_estimate))) + 
  geom_point() + 
  geom_errorbarh(aes(xmin = u5_female_pov_estimate - u5_female_pov_moe, 
                     xmax = u5_female_pov_estimate + u5_female_pov_moe), height = 0.2) + 
  scale_x_continuous(labels = comma) + 
  # facet_wrap(~year, ncol = 1) + 
  my.theme
```

```{r plot_connected_scatter, include=FALSE, fig.height=10, fig.width=8}
# connected scatter
combined_acs_data %>% 
