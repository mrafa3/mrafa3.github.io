...
```{r load_patchwork, include=TRUE, echo=FALSE}
library(patchwork)
library(maps)
```

```{r}
# Create a ggplot object from the sf object
(ggplot_object <- ggplot(data = df) +
  geom_sf(aes(fill = AREA)) +  # Use the AREA attribute to fill the polygons
  scale_fill_viridis_c() +     # Apply the viridis color scale
  theme_minimal())
