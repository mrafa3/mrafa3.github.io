...
*I'll display the code chunk options so that these can be replicated (if you are following along); for all prompts that follow, I'll hide these options for a cleaner reading experience.*
...
```{python sub4.1_python, include=FALSE}
df['salary_rank'] = df.groupby('department')['salary_annual'].rank(method='dense', ascending=False)
...
```{sql own6.2_sql, connection="con", include=TRUE}
with promotion_seq as (
      select
          name,
          salary_annual,
