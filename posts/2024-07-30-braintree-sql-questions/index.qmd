...
The BrainTree SQL coding challenge is known as a great challenge for data analysts looking to test their skills. In this project, I am putting my skills to the test, using Quarto and the `DBI::` and `RSQLite::` packages to run SQL queries on an in-memory database.
...
```{sql question7_test, connection="con", include=TRUE}
select  *
from per_capita
