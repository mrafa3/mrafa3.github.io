...
```{r}
df %>% 
  # filter(measureid == 'EMOTIONSPT') %>% 
  # filter(measureid == 'FOODSTAMP') %>%
  # filter(measureid == 'HOUSINSECU') %>%
  filter(measureid == 'FOODINSECU',
         statedesc == 'West Virginia') %>%
  # filter(measureid == 'LACKTRPT') %>%
  # filter(measureid == 'ISOLATION') %>%
  # filter(measureid == 'SHUTUTILITY') %>%
  ggplot(.) + 
  ...
  scale_fill_gradient2(
    ...
    mid = "#f0eee5",
    high = "#b6202c",
    midpoint = median(df$data_value/100, na.rm = TRUE),
    labels = scales::percent,
    name = ''
  ) + 
